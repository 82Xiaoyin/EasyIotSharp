                ...
                foreach (var tenantItem in tenant)
                {
                    ...
                    foreach (var project in projectList)
                    {
                        ...
                        try
                        {
                            ...
                            // 统计每个测点的告警数量和生成折线图数据
                            var alarmTrends = new Dictionary<string, List<int>>();
                            var dateRange = Enumerable.Range(0, 31)
                                .Select(i => startTime.AddDays(i).Date)
                                .Where(d => d < endTime.Date)
                                .ToList();
                            ...
                            foreach (var item in sensorList)
                            {
                                ...
                                var dailyAlarms = dateRange.Select(date =>
                                {
                                    return alarmData.Count(a =>
                                        sensorPointIds.Contains(a.pointid) &&
                                        a.time.ToDateTime().Date == date);
                                }).ToList();

                                alarmTrends[item.Name] = dailyAlarms;
                                var count = dailyAlarms.Sum();
                                sb.AppendLine($"告警数量为:{count}个");
                                sb.AppendLine();
                            }

                            // 生成折线图
                            var chartImage = pDFHelper.GenerateAlarmTrendChart(
                                alarmTrends,
                                dateRange.Select(d => d.ToString("MM-dd")).ToList(),
                                $"{project.Name}告警趋势图");

                            // 生成PDF报告
                            byte[] pdfBytes = pDFHelper.GenerateSimplePdf(sb.ToString(), name, chartImage);
